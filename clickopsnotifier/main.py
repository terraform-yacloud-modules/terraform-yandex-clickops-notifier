...
def get_messengers() -> List[Messenger]:
    ...
    if check_empty_envvar(slack_cc):
        slack_cc = ""
    ...
    if check_empty_envvar(telegram_cc):
        telegram_cc = ""
    ...
    if check_empty_envvar(telegram_chat_ids):
        telegram_chat_ids = []
    else:
    ...
    if telegram_messengers_len > 0:
        logging.info(
            f"{telegram_messengers_len} Telegram messengers configured.")
    else:
        logging.warning(
    ...
    for event_l1 in events["messages"]:
        for event_l2 in event_l1["details"]["messages"]:
            ...
            excluded_subject_types = os.getenv("EXCLUDED_SUBJECT_TYPES", "").split(
                ",")
            excluded_sources = os.getenv(
                "EXCLUDED_EVENT_SOURCES", "").split(",")
            excluded_types = os.getenv("EXCLUDED_EVENT_TYPES", "").split(",")

            # Apply exclusions
            if parsed_event["subject_name"] in excluded_subject_names:
                logging.debug(
                    f"Event is excluded based on EXCLUDED_SUBJECT_NAMES")
                return
            if parsed_event["subject_type"] in excluded_subject_types:
                logging.debug(
                    f"Event is excluded based on EXCLUDED_SUBJECT_TYPES")
                return
            if parsed_event["event_source"] in excluded_sources:
                logging.debug(
                    f"Event is excluded based on EXCLUDED_EVENT_SOURCES")
                return
            if parsed_event["event_type"] in excluded_types:
                logging.debug(
                    f"Event is excluded based on EXCLUDED_EVENT_TYPES")
                return
            ...
            for messenger in messengers:
                ...
                if not result_messenger:
                    logging.error(
                        f'Message NOT sent to webhook "{messenger}".')
